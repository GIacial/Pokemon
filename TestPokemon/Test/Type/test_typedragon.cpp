...
using namespace std;
//-------------------------------------------------------------------------
Test_TypeDragon::Test_TypeDragon() : QObject(){
    ...
    this->typeTest = new TypeDragon();

}
//-------------------------------------------------------------------------
Test_TypeDragon::~Test_TypeDragon() throw(){
    delete typeTest;
...
//-------------------------------------------------------------------------
void Test_TypeDragon::isWeakness(){
...
//-------------------------------------------------------------------------
void Test_TypeDragon::isResistance(){
    for(unsigned int i = 0 ; i < this->coef->size() ; i++){
        if(this->coef->at(i) < TableTypes::COEF_NORMAL){
...
//-------------------------------------------------------------------------
void Test_TypeDragon::getAttCoef(){
    for(unsigned int i = 0 ; i < this->coef->size() ; i++){
        QVERIFY(this->coef->at(i) == this->typeTest->getAttCoef(*(this->type->at(i))));
    }
}
//--------------------------------------------------------------------------
void Test_TypeDragon::isOneOfMyType(){
