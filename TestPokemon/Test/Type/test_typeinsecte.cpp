...
using namespace std;
//-------------------------------------------------------------------------
Test_TypeInsecte::Test_TypeInsecte() : QObject(){

    this->type = new vector<AbstractType*>();
    this->coef = new vector<double>();
    this->typeTest = new TypeInsecte();
...
//-------------------------------------------------------------------------
Test_TypeInsecte::~Test_TypeInsecte() throw(){
    delete typeTest;
...
//-------------------------------------------------------------------------
void Test_TypeInsecte::isWeakness(){
...
//-------------------------------------------------------------------------
void Test_TypeInsecte::isResistance(){
    for(unsigned int i = 0 ; i < this->coef->size() ; i++){
        if(this->coef->at(i) < TableTypes::COEF_NORMAL){
...
//-------------------------------------------------------------------------
void Test_TypeInsecte::getAttCoef(){
    for(unsigned int i = 0 ; i < this->coef->size() ; i++){
        QVERIFY(this->coef->at(i) == this->typeTest->getAttCoef(*(this->type->at(i))));
    }
}
//--------------------------------------------------------------------------
void Test_TypeInsecte::isOneOfMyType(){
